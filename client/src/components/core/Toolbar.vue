    ...
    <v-toolbar-items>
      <v-flex align-center layout py-2>
        ...
        <v-menu bottom left content-class="dropdown-menu" offset-y transition="slide-y-transition">
          ...
          <v-card>
            <v-list dense>
              <v-list-tile v-for="notification in notifications" @click="onClick">
...
export default {
  ...
  created() {
    ...
    db.ref("notifications").endAt().limitToLast(1).on("child_added", function(snap) {
      console.log(snap.val())
      toastr.success("You have received a new notification");
    })
  },
  beforeDestroy() {
